...
function orchestrateRelease(releaseType = "patch") {
  ...
  run(
    `npx -- release-it --no-npm --config .release-it.master.ts --dry-run --increment false --version ${baseVersion} --verbose`
  );
  ...
  run(
    `npx -- release-it --no-npm --config .release-it.dev.ts --dry-run --increment false --version ${nextDevVersion} --verbose`
  );
